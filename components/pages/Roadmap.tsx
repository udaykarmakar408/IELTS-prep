      ...
      <div className="card-blue p-6 md:p-10 relative overflow-hidden">
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8 relative z-10">
          <div className="space-y-3 md:space-y-4 text-center md:text-left">
            <div className="text-[9px] md:text-[10px] text-blue-secondary font-black uppercase tracking-[0.2em]">Your Journey</div>
            ...
            <div className="flex items-center justify-center md:justify-start gap-2">
              <p className="text-xs md:text-sm text-text-secondary font-medium tracking-wide">Targeting Band {progress.target}+</p>
              <button onClick={() => setIsEditingGoal(true)} className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-white/5 flex items-center justify-center text-blue-secondary hover:bg-blue-primary hover:text-white transition-all shadow-lg shadow-black/20">
                <Edit2 size={12} />
              </button>
            </div>
          </div>
          <button 
            ...
            className="btn btn-primary bg-violet-accent hover:bg-violet-accent/80 shadow-xl shadow-violet-accent/30 px-6 md:px-8 py-3 md:py-4 w-full md:w-auto"
          >
            {isGenerating ? <Loader2 size={18} className="animate-spin" /> : <Sparkles size={18} />}
            <span className="uppercase tracking-widest text-[10px] md:text-xs font-black">{isGenerating ? "Planning..." : "AI Weekly Plan"}</span>
          </button>
        </div>
      </div>
