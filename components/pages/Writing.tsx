...
import { WRITING_SAMPLES } from "@/lib/data/ielts_content";
...
export default function Writing() {
  ...
  useEffect(() => {
    ...
    if (words > 10) estimate = 2.0;
    if (words > 50) estimate = 3.5;
    if (words > 100) estimate = 4.5;
    if (words > 150) estimate = 5.5;
    ...
    try {
      ...
      if (progress) {
        ...
        const updated = { 
          ...
          bands: { ...progress.bands, writing: band },
          bandHistory: [...progress.bandHistory, { date: new Date().toISOString().split("T")[0], band, skill: "writing" }],
          mockHistory: [...progress.mockHistory, { date: new Date().toISOString().split("T")[0], test: `Practice: ${activeTask.title}`, band, skill: "writing" }],
        };
        setProgress(updated);
  ...
  if (activeTask) {
    ...
    if (activeTask.type === "Structure Analysis") {
      return (
        ...
        >
          <div className="flex items-center justify-between bg-bg-1/50 backdrop-blur-xl p-8 rounded-xl border border-white/5 shadow-2xl">
            <div className="flex items-center gap-8">
              <button 
                onClick={() => setActiveTask(null)} 
                className="w-14 h-14 rounded-lg bg-bg-2 text-text-muted hover:text-text-primary hover:bg-white/5 transition-all flex items-center justify-center border border-white/5 group shadow-xl"
              >
            ...
            <div className="hidden md:flex items-center gap-4 bg-blue-primary/5 px-6 py-3 rounded-lg border border-blue-primary/10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div className="space-y-6 flex flex-col h-[45vh] lg:h-[70vh]">
              <div className="card bg-bg-1/80 backdrop-blur-xl border-white/5 flex-1 overflow-y-auto custom-scrollbar p-10 rounded-xl shadow-2xl relative">
                <div className="absolute top-0 right-0 p-10 opacity-5 pointer-events-none">
                  <FileText size={150} />
                </div>
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-8">
                    <span className="tag tag-violet px-5 py-2 rounded-lg">{activeTask.type}</span>
            ...
            <div className="space-y-6 flex flex-col h-[55vh] lg:h-[70vh]">
              <div className="card p-8 flex flex-col flex-1 shadow-2xl bg-bg-1/80 backdrop-blur-xl">
                  ...
                  {activeTask.elements.map((el: any) => (
                    <div key={el.id} className="space-y-3">
                      <button
                        ...
                        className={cn(
                          "w-full p-6 rounded-xl border text-left transition-all duration-500 flex items-center justify-between group shadow-lg",
                        ...
                        {selectedElement === el.id && (
                          ...
                          >
                            <textarea
                              ...
                              className="textarea w-full bg-bg-1/50 border border-white/10 rounded-xl p-6 text-base text-text-primary outline-none focus:border-blue-primary min-h-[120px] resize-none leading-relaxed font-serif shadow-inner placeholder:text-text-muted/20"
                            />
                          </motion.div>
                        )}
                      </AnimatePresence>
                ...
                <div className="mt-auto">
                  {!structureFeedback ? (
                    <motion.button 
                      ...
                      className="group relative overflow-hidden btn btn-primary w-full py-6 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] shadow-3xl shadow-blue-primary/20"
                    >
                    ...
                    <motion.div 
                      ...
                      className="space-y-6"
                    >
                      <div className="card bg-blue-dim/20 border-blue-primary/20 p-8 shadow-2xl">
                      ...
                      <button 
                        onClick={() => setActiveTask(null)} 
                        className="btn btn-ghost w-full py-5 rounded-xl text-[10px] font-black uppercase tracking-widest border-white/5"
                      >
    ...
    return (
      ...
      >
        {/* Practice Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-bg-1/50 backdrop-blur-xl p-8 rounded-xl border border-white/5 shadow-2xl">
          <div className="flex items-center gap-8">
            <button 
              onClick={() => setActiveTask(null)}
              className="w-16 h-16 rounded-lg bg-bg-2 text-text-muted hover:text-text-primary hover:bg-white/5 transition-all flex items-center justify-center border border-white/5 shadow-xl group"
            >
          ...
          <div className="flex items-center gap-6">
            <button 
              ...
              className={cn(
                "px-8 py-4 rounded-lg border transition-all flex items-center gap-3 text-[10px] font-black uppercase tracking-widest shadow-2xl",
                isFocusMode 
            ...
            <div className="recipe-hardware-widget flex items-center gap-6 px-8 py-4 bg-bg-2/80 backdrop-blur-xl border-white/10 rounded-lg shadow-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 h-full">
          ...
          <div className="space-y-8 flex flex-col h-[40vh] lg:h-full">
            <div className="relative overflow-hidden card bg-bg-1/80 backdrop-blur-xl border-white/5 flex-1 overflow-y-auto custom-scrollbar p-10 rounded-xl shadow-2xl">
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-10">
                  <span className="tag tag-blue px-6 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg bg-blue-primary/10 border-blue-primary/20">
                    {activeTask.type}
                  </span>
                  <div className="flex items-center gap-3 text-[10px] font-black text-text-muted uppercase tracking-widest bg-bg-2/50 px-5 py-2 rounded-lg border border-white/5">
                ...
                {activeTask.chartData && (
                  <div className="mb-10 p-8 bg-bg-2/50 backdrop-blur-xl rounded-lg border border-white/5 shadow-inner group">
                ...
                {activeTask.image && !activeTask.chartData && (
                  <div className="relative aspect-video rounded-lg overflow-hidden border border-white/10 mb-10 shadow-3xl group">
                    <img 
          ...
          <div className="space-y-8 flex flex-col h-[60vh] lg:h-full">
            <div className="card border-white/5 p-0 overflow-hidden flex flex-col flex-1 rounded-xl shadow-3xl bg-bg-1/80 backdrop-blur-xl">
              <div className="bg-bg-2/50 backdrop-blur-xl px-10 py-6 border-b border-white/5 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-4 bg-blue-primary/5 px-6 py-2 rounded-lg border border-blue-primary/10">
              ...
              <textarea
                ...
                className="textarea flex-1 bg-transparent p-12 text-xl outline-none resize-none leading-[1.8] custom-scrollbar font-serif text-text-primary placeholder:text-text-muted/20 selection:bg-blue-primary/20"
              />
            </div>
            ...
            {!feedback ? (
              <motion.button 
                ...
                className="group relative overflow-hidden btn btn-primary w-full py-8 rounded-lg text-xs font-black uppercase tracking-[0.3em] shadow-3xl shadow-blue-primary/20"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
              ...
              <div className="space-y-8 flex-1 flex flex-col min-h-0">
                {/* Feedback Tabs */}
                <div className="flex bg-bg-2/50 backdrop-blur-xl p-2 rounded-xl border border-white/5 shrink-0 shadow-2xl">
                  {[
                    ...
                    <button 
                      ...
                      className={cn(
                        "flex-1 py-4 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-500",
                        feedbackTab === tab.id 
                ...
                <div className="flex-1 min-h-0">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {feedbackTab === "report" ? (
                        <div className="card bg-bg-1/80 backdrop-blur-xl border-white/5 h-full flex flex-col overflow-hidden rounded-xl p-0 shadow-3xl">
                          <div className="p-10 border-b border-white/5 flex items-center justify-between shrink-0 bg-blue-primary/5">
                            <div className="flex items-center gap-4 text-blue-secondary font-black text-[12px] uppercase tracking-[0.3em]">
                              <div className="w-10 h-10 rounded-lg bg-blue-primary/10 flex items-center justify-center">
                                <MessageSquare size={20} />
                              </div>
                              AI Examiner Report
                            </div>
                            {feedback?.overallBand && (
                              <div className="flex items-center gap-6 bg-blue-primary text-white px-8 py-3 rounded-lg shadow-3xl shadow-blue-primary/30">
                          ...
                          <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-12">
                            ...
                            {feedback?.criteria && (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {Object.entries(feedback.criteria).map(([key, data]: [string, any]) => (
                                  <div key={key} className="p-8 bg-bg-2/30 backdrop-blur-xl border border-white/5 rounded-lg space-y-6 shadow-inner group hover:border-blue-primary/20 transition-all">
                            ...
                            {feedback?.keyImprovement && (
                              <div className="p-10 bg-violet-accent/5 border border-violet-accent/10 rounded-xl relative overflow-hidden shadow-inner group">
                              ...
                              <div className="space-y-6">
                                {userText.split('\n\n').map((para, idx) => (
                                  <div key={idx} className="flex gap-8 group">
                                    ...
                                    <div className="flex-1 p-8 bg-bg-2/30 rounded-xl border border-white/5 text-base text-text-muted group-hover:border-blue-primary/20 group-hover:bg-bg-2/50 transition-all duration-500 shadow-inner">
                        ...
                        <div className="card bg-bg-1/80 backdrop-blur-xl border-white/5 h-full flex flex-col rounded-xl p-0 overflow-hidden shadow-3xl">
                          ...
                          <div className="overflow-y-auto p-10 custom-scrollbar flex-1 space-y-12">
                            <div className="p-10 bg-bg-2/30 rounded-lg border border-white/5 shadow-inner text-2xl leading-[1.8] font-serif whitespace-pre-wrap text-text-primary italic opacity-90">
                              {smartReview ? (
                              ...
                              <div className="grid grid-cols-1 gap-6">
                                {smartReview?.map((err, i) => (
                                  <motion.div 
                                    ...
                                    className="p-8 bg-bg-2/50 border border-white/5 rounded-xl flex items-start gap-8 group hover:border-amber-accent/30 transition-all duration-500 shadow-inner"
                                  >
                                    <div className={cn(
                        ...
                        <div className="card bg-bg-1/80 backdrop-blur-xl border-white/5 h-full flex flex-col rounded-xl p-0 overflow-hidden shadow-3xl">
                          <div className="p-10 border-b border-violet-accent/10 flex items-center gap-4 text-violet-accent font-black text-[12px] uppercase tracking-[0.3em] bg-violet-accent/5">
                            <div className="w-10 h-10 rounded-lg bg-violet-accent/10 flex items-center justify-center">
                              <Trophy size={20} />
                            </div>
                ...
                <button 
                  ...
                  className="btn btn-ghost w-full py-6 rounded-xl text-[11px] font-black uppercase tracking-[0.3em] hover:bg-white/5 transition-all duration-500 border-white/5"
                >
                  Try Another Task
                </button>
              </div>
  ...
  return (
    <div className="space-y-16 pb-20">
      {/* Hero Header */}
      <div className="relative overflow-hidden rounded-xl bg-bg-1 border border-white/5 p-8 md:p-12 lg:p-16">
        ...
        <div className="relative z-10 flex flex-col 2xl:flex-row 2xl:items-end justify-between gap-12">
          ...
          <motion.div 
            ...
            className="flex flex-wrap md:flex-nowrap bg-bg-2/50 backdrop-blur-xl p-2 rounded-xl border border-white/5 shadow-2xl"
          >
            {[
              ...
              <button 
                ...
                className={cn(
                  "flex items-center gap-2 px-8 py-4 rounded-lg text-[11px] font-black uppercase tracking-widest transition-all duration-500",
                  activeTab === tab.id 
      ...
      {activeTab === "practice" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {/* Dynamic AI Task Generator Card */}
          <motion.button
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            onClick={() => setActiveTab("ai-test")}
            className="group relative overflow-hidden card w-full text-left p-10 rounded-xl transition-all hover:shadow-3xl hover:shadow-blue-primary/20 border-blue-primary/30 bg-blue-primary/5"
          >
            <div className="absolute inset-0 bg-gradient-to-br from-blue-primary/10 to-transparent opacity-100 transition-opacity duration-500" />
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8">
                <span className="tag tag-blue px-5 py-2 text-[10px] font-black rounded-lg bg-blue-primary text-white">DYNAMIC AI</span>
                <Sparkles size={20} className="text-blue-primary animate-pulse" />
              </div>
              <h3 className="recipe-editorial-h1 text-3xl mb-6 text-blue-primary">Generate Custom Task</h3>
              <p className="text-lg text-text-muted mb-10 font-medium leading-relaxed">
                Let Aria synthesize a unique writing prompt tailored specifically to your current progress and target band score.
              </p>
              <div className="flex items-center justify-between pt-8 border-t border-blue-primary/10">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg bg-blue-primary/20 flex items-center justify-center text-blue-primary">
                    <PenTool size={18} />
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-blue-secondary">Infinite Practice</span>
                </div>
                <div className="w-14 h-14 rounded-lg bg-blue-primary text-white flex items-center justify-center shadow-lg group-hover:translate-x-2 transition-transform">
          ...
          {WRITING_TASKS.map((task, idx) => (
            <motion.button
              ...
              className="group relative overflow-hidden card w-full text-left p-10 rounded-xl transition-all hover:shadow-3xl hover:shadow-blue-primary/10 border-white/5 hover:border-blue-primary/30"
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <span className={cn(
                    "tag px-5 py-2 text-[10px] font-black rounded-lg",
                  ...
                  <div className="flex items-center gap-4 text-[10px] font-black text-text-muted uppercase tracking-widest">
                    <div className="flex items-center gap-2 bg-bg-2 px-4 py-2 rounded-lg border border-white/5">
                ...
                <div className="flex items-center justify-between pt-8 border-t border-white/5">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-blue-primary/10 flex items-center justify-center text-blue-primary">
                      <FileText size={18} />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">{task.wordCount}+ Words</span>
                  </div>
                  <div className="w-14 h-14 rounded-lg bg-bg-2 flex items-center justify-center group-hover:bg-blue-primary group-hover:text-white transition-all duration-500 group-hover:translate-x-2 shadow-lg">
                    <ChevronRight size={24} />
                  </div>
                </div>
              </div>
      ...
      {activeTab === "samples" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {WRITING_SAMPLES.map((sample, idx) => (
            ...
            >
              <div className="absolute inset-0 bg-gradient-to-br from-blue-primary/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-xl blur-3xl" />
              <div className="card h-full flex flex-col p-10 bg-bg-1/80 backdrop-blur-xl border-white/5 rounded-xl shadow-2xl hover:shadow-blue-primary/10 transition-all relative overflow-hidden">
                ...
                <div className="flex items-center justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-lg bg-blue-primary/10 flex items-center justify-center text-blue-primary">
                      <Trophy size={20} />
                    </div>
                    <div>
                  ...
                  <span className="tag tag-blue px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest">{sample.type}</span>
                </div>

                <div className="space-y-8">
                  <div className="p-8 bg-bg-2/50 rounded-xl border border-white/5 shadow-inner">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-8 bg-bg-1/50 rounded-xl text-base text-text-muted leading-[1.8] border border-white/5 font-serif shadow-inner h-[300px] overflow-y-auto custom-scrollbar pr-6">
                      {sample.modelAnswer}
                    </div>
                  </div>

                  <div className="p-8 bg-blue-primary/5 border border-blue-primary/10 rounded-xl shadow-xl">
      ...
      {activeTab === "ai-test" && (
        <div className="relative overflow-hidden rounded-xl bg-bg-1 border border-white/5 p-20 text-center">
          ...
          <div className="relative z-10 flex flex-col items-center space-y-10">
            <motion.div 
              ...
              className="w-32 h-32 rounded-lg bg-blue-primary/10 text-blue-primary flex items-center justify-center shadow-3xl shadow-blue-primary/10 border border-blue-primary/20"
            >
              <Sparkles size={60} />
            </motion.div>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              <button 
                ...
                className="group relative overflow-hidden btn btn-secondary px-12 py-6 rounded-lg text-xs font-black uppercase tracking-[0.3em] shadow-3xl shadow-blue-primary/10"
              >
                <div className="flex items-center gap-4">
              ...
              <button 
                ...
                disabled={isGenerating}
                className="group relative overflow-hidden btn btn-primary px-12 py-6 rounded-lg text-xs font-black uppercase tracking-[0.3em] shadow-3xl shadow-blue-primary/30"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-1000" />
                <div className="flex items-center gap-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {STRUCTURE_TASKS.map((task, idx) => (
            <motion.button
              ...
              transition={{ delay: idx * 0.1 }}
              onClick={() => startTask(task)}
              className="group relative overflow-hidden card w-full text-left p-10 rounded-xl transition-all hover:shadow-3xl hover:shadow-violet-accent/10 border-white/5 hover:border-violet-accent/30 bg-gradient-to-br from-violet-accent/5 to-bg-1"
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <span className="tag tag-violet px-5 py-2 text-[10px] font-black rounded-lg uppercase tracking-widest">{task.type}</span>
                  <div className="flex items-center gap-4 text-[10px] font-black text-text-muted uppercase tracking-widest">
                    <div className="flex items-center gap-2 bg-bg-2 px-4 py-2 rounded-lg border border-white/5">
                ...
                <div className="flex items-center justify-between pt-8 border-t border-white/5">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-violet-accent/10 flex items-center justify-center text-violet-accent">
                      <Target size={18} />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">{task.difficulty}</span>
                  </div>
                  <div className="w-14 h-14 rounded-lg bg-bg-2 flex items-center justify-center group-hover:bg-violet-accent group-hover:text-white transition-all duration-500 group-hover:translate-x-2 shadow-lg">
                    <ChevronRight size={24} />
                  </div>
                </div>
              </div>
