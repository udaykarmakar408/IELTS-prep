...
export default function Dashboard({ setActivePage }: DashboardProps) {
  ...
  const leaderboard = [
    { name: "Sarah J.", band: 8.5, rank: 1, avatar: "SJ" },
    { name: "Alex M.", band: 8.0, rank: 2, avatar: "AM" },
    { name: "You", band: avgBand === "0.0" ? 0 : parseFloat(avgBand), rank: 42, avatar: progress.name.substring(0, 2).toUpperCase(), isUser: true },
    { name: "Elena R.", band: 7.5, rank: 4, avatar: "ER" },
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-serif font-black text-text-primary tracking-tight">Global Leaderboard</h3>
                <p className="text-[10px] text-text-muted font-bold uppercase tracking-widest">Top students this week</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2 px-3 py-1 bg-blue-primary/10 rounded-full border border-blue-primary/20">
                <div className="w-2 h-2 bg-blue-primary rounded-full animate-pulse" />
                <span className="text-[9px] font-black text-blue-secondary uppercase tracking-widest">1,248 Online</span>
              </div>
              <div className="flex items-center gap-2 px-3 py-1 bg-green-accent/10 rounded-full border border-green-accent/20">
                <div className="w-2 h-2 bg-green-accent rounded-full" />
                <span className="text-[9px] font-black text-green-accent uppercase tracking-widest">Live Activity</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {leaderboard.map((user, i) => (
              <motion.div 
                ...
                className={cn(
                  "flex items-center justify-between p-4 rounded-2xl border transition-all group hover:shadow-xl hover:scale-[1.01]",
                  user.isUser ? "bg-blue-primary/5 border-blue-primary/30 shadow-lg" : "bg-bg-1 border-border-2"
                )}
              >
                <div className="flex items-center gap-4">
                  <div className="w-8 flex flex-col items-center">
                    <div className="font-black text-text-muted text-sm">#{user.rank}</div>
                    {i < 3 && (
                      <motion.div 
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className={cn(
                          ...
                          i === 0 ? "text-amber-accent" : i === 1 ? "text-slate-400" : "text-amber-700"
                        )}
                      >
                  ...
                  <div className={cn(
                    "w-10 h-10 rounded-full flex items-center justify-center text-xs font-black relative overflow-hidden ring-2 ring-offset-2 ring-offset-bg-1",
                    user.isUser ? "bg-blue-primary text-white ring-blue-primary/50" : "bg-bg-3 text-text-secondary ring-transparent"
                  )}>
                    {user.avatar}
                    {i < 3 && <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent" />}
                  </div>
                  <div>
                    <div className="text-sm font-bold text-text-primary flex items-center gap-2">
                      {user.name} {user.isUser && "(You)"}
                      {i === 0 && <Star size={12} className="text-amber-accent fill-amber-accent animate-spin-slow" />}
                    </div>
                    <div className="text-[10px] text-text-muted font-medium flex items-center gap-2">
                      <span className="w-1.5 h-1.5 bg-green-accent rounded-full animate-pulse" /> 
                      Studying {["Listening", "Speaking", "Writing", "Reading", "Vocabulary"][i % 5]}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-6">
                  <div className="hidden md:flex flex-col items-end">
                    <div className="flex items-center gap-1 text-green-accent text-[10px] font-bold">
                      <TrendingUp size={10} /> +{Math.floor(Math.random() * 5) + 1}
                    </div>
                    <div className="text-[8px] text-text-muted font-black uppercase tracking-widest">Weekly Trend</div>
                  </div>
                  <div className="text-right">
                    <div className="text-lg font-black text-blue-secondary">Band {user.band || "—"}</div>
                    <div className="text-[8px] text-text-muted font-bold uppercase tracking-widest">Predicted Score</div>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        <motion.div 
          className="card p-8 bg-bg-2 border-border-2 flex flex-col relative overflow-hidden group"
          variants={{
            hidden: { opacity: 0, x: 20 },
            visible: { opacity: 1, x: 0 }
          }}
        >
          <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
            <Users size={80} className="text-violet-accent" />
          </div>
          <div className="relative z-10 flex flex-col h-full">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-violet-accent/10 rounded-full flex items-center justify-center text-violet-accent shadow-inner">
                  <Users size={20} />
                </div>
                <div>
                  <div className="text-[10px] text-text-muted font-black uppercase tracking-widest">Community</div>
                  <div className="text-lg font-serif font-black text-text-primary uppercase tracking-tighter">Live Study Room</div>
                </div>
              </div>
              <div className="flex items-center gap-2 px-3 py-1 bg-red-accent/10 rounded-full border border-red-accent/20">
                <div className="w-2 h-2 bg-red-accent rounded-full animate-pulse" />
                <span className="text-[10px] font-black text-red-accent uppercase tracking-widest">4 Rooms Active</span>
              </div>
            </div>
            
            <div className="flex-1 space-y-6">
              <div className="relative aspect-video bg-bg-1 rounded-3xl border border-border-2 overflow-hidden group shadow-2xl">
                  ...
                  <div className="grid grid-cols-3 gap-3 p-4 w-full h-full">
                    {[1, 2, 3, 4, 5, 6].map(i => (
                      <motion.div 
                        ...
                        className="bg-bg-3 rounded-2xl flex items-center justify-center relative overflow-hidden border border-white/5 shadow-sm"
                      >
                        <div className="w-10 h-10 rounded-full bg-bg-2 flex items-center justify-center text-lg font-black shadow-inner">
                          {["👨‍🎓", "👩‍🎓", "🧑‍🎓", "👨‍💻", "👩‍💻", "🧑‍💻"][i-1]}
                        </div>
                        <div className="absolute bottom-2 right-2 w-2.5 h-2.5 bg-green-accent rounded-full border-2 border-bg-3 shadow-[0_0_8px_rgba(34,197,94,0.6)]" />
                      </motion.div>
                    ))}
                  </div>
                </div>
                ...
                <div className="absolute bottom-4 left-4 right-4 pointer-events-none">
                  <motion.div 
                    ...
                    className="bg-black/40 backdrop-blur-md p-3 rounded-2xl border border-white/10 max-w-[80%] shadow-xl"
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[8px] font-black text-blue-secondary uppercase tracking-widest">Sarah J.</span>
                      <span className="text-[7px] text-white/50 uppercase tracking-widest">Just now</span>
                    </div>
                    <p className="text-[9px] text-white/90 font-medium leading-tight">
                      &quot;Anyone want to practice Speaking Part 2?&quot;
                    </p>
                  </motion.div>
                </div>

                <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 backdrop-blur-[4px]">
                  <div className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center mb-4 border border-white/20 animate-pulse">
                    <Play size={24} className="text-white ml-1" />
                  </div>
                  <p className="text-[10px] font-black text-white mb-4 uppercase tracking-widest drop-shadow-lg">Room #402: Advanced Writing</p>
                  <button className="btn btn-primary py-3 px-8 text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-blue-primary/50 hover:scale-105 active:scale-95 transition-all">Join Session</button>
                </div>
                
                <div className="absolute top-4 left-4 px-3 py-1.5 bg-red-accent/90 rounded-full text-[9px] font-black text-white uppercase tracking-widest flex items-center gap-2 shadow-xl border border-white/20">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-xs text-text-secondary leading-relaxed max-w-[200px] font-medium">
                    Join <span className="text-text-primary font-bold">12 others</span> in a focused study session.
                  </p>
                  <div className="flex flex-col items-end">
                    <div className="text-[10px] font-black text-green-accent flex items-center gap-1">
                      <TrendingUp size={12} /> +23%
                    </div>
                    <div className="text-[8px] text-text-muted font-bold uppercase tracking-widest">Retention</div>
                  </div>
                </div>
                  ...
                  <div className="flex -space-x-3">
                    {[1, 2, 3, 4, 5].map(i => (
                      <motion.div 
                        ...
                        className="w-10 h-10 rounded-full border-2 border-bg-2 bg-bg-3 flex items-center justify-center text-lg shadow-lg cursor-pointer transition-transform"
                      >
                        {["👤", "👥", "👤", "👥", "👤"][i-1]}
                      </motion.div>
                    ))}
                    <div className="w-10 h-10 rounded-full border-2 border-bg-2 bg-blue-primary flex items-center justify-center text-[10px] font-black text-white shadow-xl">
                      +7
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-[8px] font-black text-text-muted uppercase tracking-widest mb-1">Next Session</span>
                    <div className="flex items-center gap-2 px-3 py-1 bg-bg-3 rounded-lg border border-border-2">
                      <Clock size={12} className="text-blue-secondary" />
                      <span className="text-[10px] font-black text-text-primary uppercase tracking-widest">In 14m</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-bg-1/50 rounded-xl p-3 border border-border-2 space-y-2">
              <div className="flex items-center gap-2">
                <span className="text-[8px] font-black text-blue-secondary uppercase">Aria (AI)</span>
                <span className="text-[8px] text-text-muted">Just now</span>
              </div>
              <p className="text-[10px] text-text-secondary italic line-clamp-1">
                "Great job everyone! Let's focus on the next cue card..."
              </p>
            </div>
          </div>
          
          <div className="mt-8 pt-6 border-t border-border-2">
            <div className="flex items-center gap-3 p-3 bg-violet-accent/5 rounded-xl border border-violet-accent/10">
              <Lightbulb size={14} className="text-violet-accent" />
              <p className="text-[10px] text-text-secondary leading-tight">
                Focus on <b>Complex Sentences</b> today.
              </p>
            </div>
          </div>
          </div>
