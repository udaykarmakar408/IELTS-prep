    ...
    <div className="space-y-8">
      <div className="card-blue p-8 md:p-10 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="text-[10px] text-blue-secondary font-black uppercase tracking-[0.2em]">Speaking Lab</div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <div className="card border-blue-primary/30 bg-bg-2 p-8">
                <div className="text-[10px] text-blue-secondary font-black uppercase tracking-[0.2em] mb-4">IELTS Speaking Part 2</div>
                <h3 className="font-serif text-3xl font-black text-text-primary mb-6 leading-tight">
                  {cueCardData.topic}
                </h3>
                <div className="space-y-4">
                  <p className="text-sm font-bold text-text-muted uppercase tracking-widest">You should say:</p>
                  ...
                  <p className="text-xs text-text-muted italic mt-6 pt-6 border-t border-border/50">
              ...
              {phase === "speak" && (
                <div className="card bg-red-accent/5 border-red-accent/20 flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-20 h-20 bg-red-accent rounded-full flex items-center justify-center text-white mb-6 animate-pulse shadow-2xl shadow-red-accent/40">
                    <Mic size={40} />
                  </div>
                  <h4 className="text-xl font-serif font-black text-text-primary mb-2">Recording in Progress</h4>
                  <p className="text-xs text-text-muted uppercase tracking-widest font-bold">Speak for 1-2 minutes</p>
                  <button 
                    onClick={() => { setIsTimerActive(false); setTimer(0); }}
                    className="btn btn-ghost mt-8 border-red-accent/30 text-red-accent hover:bg-red-accent hover:text-white"
                  >
              ...
              {phase === "prep" && (
                <div className="card bg-amber-accent/5 border-amber-accent/20 flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-20 h-20 bg-amber-accent rounded-full flex items-center justify-center text-white mb-6 shadow-2xl shadow-amber-accent/40">
                    <Clock size={40} />
                  </div>
                  <h4 className="text-xl font-serif font-black text-text-primary mb-2">Preparation Time</h4>
                  <p className="text-xs text-text-muted uppercase tracking-widest font-bold">Take notes on your structure</p>
                  <button 
                    onClick={() => { setPhase("speak"); setTimer(120); }}
                    className="btn btn-primary bg-amber-accent hover:bg-amber-accent/80 border-none mt-8"
                  >
                    Start Speaking Now
                  </button>
                </div>
              ...
              {timer === 0 && phase === "speak" && !feedback && (
                <div className="card bg-green-accent/5 border-green-accent/20 flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-20 h-20 bg-green-accent rounded-full flex items-center justify-center text-white mb-6 shadow-2xl shadow-green-accent/40">
                    <Trophy size={40} />
                  </div>
                  <h4 className="text-xl font-serif font-black text-text-primary mb-2">Time&apos;s Up!</h4>
                  <p className="text-xs text-text-muted uppercase tracking-widest font-bold">Great job completing the session</p>
                  <button 
                    onClick={getAIAnalysis}
                    disabled={isAnalyzing}
                    className="btn btn-primary bg-green-accent hover:bg-green-accent/80 border-none mt-8 w-full max-w-xs"
                  >
